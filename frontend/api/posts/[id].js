/* ═══════════════════════════════════════════════════════════
   HUMANITAID — Vercel Serverless Function
   GET    /api/posts/:id  — public si published, admin sinon
   PUT    /api/posts/:id  — admin (editor+), mise à jour
   DELETE /api/posts/:id  — admin (editor+), corbeille (soft
                             delete : deleted_at = NOW())

   Le hard delete n'existe pas ici par choix : deleted_at permet
   la restauration (POST /api/posts/:id/restore) exactement comme
   l'attend admin/js/publications.js (bouton "Restaurer").
   ═══════════════════════════════════════════════════════════ */

const { redact, buildPgOptions, getOptionalAdmin, requireAdmin } = require('../_shared');
const { buildUpdateQuery, isValidUUID } = require('../_sanitizers');
const { validatePost } = require('../_validation');

const ADMIN_COLUMNS =
  'id, title, slug, summary, content, featured_image, gallery, video_url, category, location, cause_id, is_featured, status, published_at, scheduled_at, seo_title, seo_description, og_image, view_count, deleted_at, created_at, updated_at';
const PUBLIC_COLUMNS =
  'id, title, slug, summary, content, featured_image, video_url, category, location, is_featured, status, published_at, created_at, updated_at';

async function handleGet(req, res, pool, id) {
  const admin = getOptionalAdmin(req);
  const columns = admin ? ADMIN_COLUMNS : PUBLIC_COLUMNS;
  const byId = isValidUUID(id);
  const cond = byId ? 'id = $1' : 'slug = $1';

  const { rows } = await pool.query(`SELECT ${columns} FROM posts WHERE ${cond} LIMIT 1`, [id]);
  const post = rows[0];

  if (!post) return res.status(404).json({ error: 'Introuvable' });
  if (!admin && (post.status !== 'published' || post.deleted_at)) {
    return res.status(404).json({ error: 'Introuvable' });
  }
  return res.json({ post });
}

async function handlePut(req, res, pool, id) {
  const admin = requireAdmin(req, res, 'editor');
  if (!admin) return;
  if (!isValidUUID(id)) return res.status(400).json({ error: 'Identifiant invalide' });

  const body = req.body || {};
  const errors = validatePost(body, { partial: true });
  if (errors.length) return res.status(400).json({ error: 'Données invalides', details: errors });

  const payload = { ...body };
  if (payload.cause_id === '') payload.cause_id = null;

  const update = buildUpdateQuery('posts', id, payload);
  if (!update) return res.status(400).json({ error: 'Aucune donnée exploitable' });

  try {
    const { rows } = await pool.query(update.sql, update.values);
    if (!rows[0]) return res.status(404).json({ error: 'Introuvable' });
    return res.json({ post: rows[0] });
  } catch (err) {
    if (err && err.code === '23505') {
      return res.status(409).json({ error: 'Ce slug existe déjà, choisissez-en un autre' });
    }
    throw err;
  }
}

async function handleDelete(req, res, pool, id) {
  const admin = requireAdmin(req, res, 'editor');
  if (!admin) return;
  if (!isValidUUID(id)) return res.status(400).json({ error: 'Identifiant invalide' });

  const { rows } = await pool.query(
    'UPDATE posts SET deleted_at = NOW() WHERE id = $1 AND deleted_at IS NULL RETURNING id',
    [id]
  );
  if (!rows[0]) return res.status(404).json({ error: 'Introuvable ou déjà dans la corbeille' });
  return res.json({ ok: true });
}

module.exports = async function postById(req, res) {
  const databaseUrl = process.env.DATABASE_URL;
  if (!databaseUrl) {
    console.error('[posts/:id] DATABASE_URL manquant');
    return res.status(503).json({ error: 'Service temporairement indisponible.' });
  }
  const { id } = req.query || {};
  if (!id) return res.status(400).json({ error: 'Identifiant manquant' });

  let pool = null;
  try {
    const { Pool } = require('pg');
    pool = new Pool(buildPgOptions(databaseUrl));

    if (req.method === 'GET') return await handleGet(req, res, pool, id);
    if (req.method === 'PUT') return await handlePut(req, res, pool, id);
    if (req.method === 'DELETE') return await handleDelete(req, res, pool, id);
    return res.status(405).json({ error: 'Méthode non autorisée' });
  } catch (err) {
    console.error('[posts/:id] Erreur:', redact(err && err.message ? err.message : err));
    return res.status(500).json({ error: 'Erreur serveur' });
  } finally {
    if (pool) await pool.end().catch(() => {});
  }
};
